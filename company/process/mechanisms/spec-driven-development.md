...
**Owner:** Principal
**Location:** `governance/mechanisms/spec-driven-development.md`
**Companion:** `governance/controls/ctrl-spec-profiles.md` (domain-specific phase guidance)
...
Process artifacts → `library/`. Retired specs → `library/`. Reference materials → `reference/`.
...
5. Write analysis to `library/` (feeds the spec but is not the spec)
...
| **Process artifacts in specs/** | Trackers and drafts clutter the spec folder | `specs/` for current spec only, `library/` for everything else |
...
This process produces the **input** to the Autonomous Agent SDLC (`governance/mechanisms/autonomous-sdlc.md`). The spec suite is the reading list for Sprint 0 and all subsequent development sprints. The SDLC consumes the spec; this process produces it.

```
SDD (this process)          →  Spec Suite  →  SDLC (autonomous-sdlc.md)
governance/mechanisms/         specs/           governance/mechanisms/
