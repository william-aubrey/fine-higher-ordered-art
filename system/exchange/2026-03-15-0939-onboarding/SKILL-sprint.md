...
If `library/sprints/` does not exist, create it (and `library/` if needed) before writing.
...
10. **Stop.** Do not write code, make changes, or execute tasks.
...
4. **Review the session.** Scan the full conversation for:
