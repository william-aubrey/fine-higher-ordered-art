...
4. If no products exist → ask: "What is the product name?" and create `company/production/[product-name]/define/`.
5. Store the active product in `spec-progress.md` so subsequent invocations don't re-ask.

All spec paths below use `company/production/[product]/define/` as the base.
...
| Item | Path | Purpose |
|---|---|---|
| Spec documents | `company/production/[product]/define/spec-v1-*.md` | The deliverable — numbered spec files |
| Spec index | `company/production/[product]/define/spec-v1-00-index.md` | Reading order + status |
| Progress tracker | `company/production/[product]/define/spec-progress.md` | Phase completion, gap tracking |
...
2. **Check for existing spec suite.** If `company/production/[product]/define/spec-progress.md` exists, report: "Spec suite already initialized. Run `/spec status` to see progress, or `/spec plan` to revise." Stop.
...
7. **Write the progress tracker.** After Principal approval, create `company/production/[product]/define/spec-progress.md`:
...
8. **Create the spec index.** Create `company/production/[product]/define/spec-v1-00-index.md` with the planned document list, reading order, and empty sections for key decisions and supporting references.

9. **Create folders** if they don't exist: `company/production/[product]/define/`.
...
2. **Read the progress tracker** (`company/production/[product]/define/spec-progress.md`). Verify this phase's prerequisites are met:
...
9. **Update the progress tracker.** Mark documents as complete in `company/production/[product]/define/spec-progress.md` with the current date.

10. **Update the spec index.** Add or update entries in `company/production/[product]/define/spec-v1-00-index.md`.
...
7. **Update the progress tracker.** Write gaps to the Gap Tracker table in `company/production/[product]/define/spec-progress.md`. Log the simulation pass in the Simulation Log.
...
9. **If zero dangerous gaps,** mark the simulate phase as complete in `company/production/[product]/define/spec-progress.md`.
...
1. Resolve the product, then read `company/production/[product]/define/spec-progress.md`.
...
**Rule: `ls` must produce the reading order.** If someone types `ls company/production/[product]/define/` the files appear in the order they should be read.
...
- **Spec folder hygiene.** `company/production/[product]/define/` contains only current, authoritative spec documents. Working drafts, exploratory analysis, and experimental code go in `company/production/[product]/learn/`. Agent-portable research (not product-bound) goes in `agents/[agent]/products/`. Reference materials go in `agents/librarian/products/library/make/` or `company/process/functions/`.
