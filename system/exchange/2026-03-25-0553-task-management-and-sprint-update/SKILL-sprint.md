...
| Task backlog | `TASKS.md` (project root) | What needs to be done (headers match repo ontology) |
| Completed tasks | `system/log/completed-tasks.csv` | Append-only archive of completed tasks |
...
6. **Read TASKS.md.** If it exists, extract:
   ...
   - Groupings (H2/H3 headers matching repo folder ontology)

   TASKS.md contains only open tasks — no completed items, no priority tags. Completed tasks live in `system/log/completed-tasks.csv`.
...
7. **Update TASKS.md and completed-tasks archive.**

   **TASKS.md structure:** Headers match the repo folder ontology (H2 = top-level folders, H3 = subfolders). Only open tasks live here. No completed items.

   For each task completed during this session:
   1. **Remove** the `- [ ]` line from TASKS.md
   2. **Append** a row to `system/log/completed-tasks.csv`:
      ```
      completed_date,sprint_id,area,task,narrative
      ```
      - `completed_date`: YYYY-MM-DD
      - `sprint_id`: current sprint (e.g., SPR-031)
      - `area`: the TASKS.md header path (e.g., `agents/cao`, `enterprise/production`)
      - `task`: the task text (no markdown checkbox, quote commas)
      - `narrative`: the sprint narrative filename

   For new tasks discovered during the session:
   - Add them under the appropriate ontology header in TASKS.md
   - If no matching header exists, create one that matches the repo folder path

   If TASKS.md doesn't exist and tasks were identified, create it with ontology-matched headers.
