...
- **Spec folder hygiene.** `company/production/[product]/specs/` contains only current, authoritative spec documents. Working drafts, prototype analysis, and exploratory code go in `company/production/[product]/prototype/`. Agent-portable research (not product-bound) goes in `agents/[agent]/library/`. Reference materials go in `company/process/mechanisms/`.
