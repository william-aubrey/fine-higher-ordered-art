...
```
c3po-model-version:     2026-03-22
enterprise-model-rev:   R1.2a (anti-gerund rule in effect) + R5.3 (instance boundary authority)
derived-from:           enterprise/production/eaas/build/model/enterprise-model.md
                        enterprise/process/controls/ctrl-constraint-library.md
                        enterprise/process/controls/ctrl-mechanism-catalog.md
                        enterprise/process/interfaces/ctrl-interface-register.md
                        enterprise/process/controls/ctrl-instance-governance-standard.md
...
governance-model:       Boundary-enforcement (compliance measured at A-0 interface)
agent-roster:           9 CXO agents + Librarian (infrastructure)
...
│
├── System Layer          system/               ← sprints, logs, transcripts, archive
...
│   ├── process/          ← governing standards (this file lives here)
...
*Source: enterprise/c3po-soul.md — read-only, refreshed only by Principal*
...
*Source: enterprise/c3po-soul.md*
...
### P10 — Librarian (Infrastructure Agent)

**Source:** agents/librarian/ (established 2026-03-22)
**IDEF0 position:** Not an enterprise function node. Infrastructure agent — operates at the C3PO/instance level outside the enterprise model, like the CAO.

**Mandate:** Curate, catalog, and maintain the instance's reference library. Accept incoming deposits (push) and fetch external resources (pull). Process raw materials into AI-friendly markdown reference files using per-shelf spec templates. Maintain organized shelves: standards, voices, whitepapers, youtubers, and any future categories.

**Products:** The curated reference library (`agents/librarian/products/library/build/`). Each named product follows the `{build, prototype, specs}` triad.

**Processes:** Intake and cataloging procedures. Each shelf type has a spec template defining the output format (YAML frontmatter, required sections, metadata fields).

**In secondary repos:** The Librarian is instantiated at onboarding alongside the CAO. It manages all reference material — standards, external analyses, publications. The Librarian does not own governance documents (those live in `company/process/`) or agent work products (those live in `agents/cXo/products/`).
...
|---|---|---|
| Decomposition Rules | enterprise/process/controls/ctrl-decomposition-rules.md | 17 rules + R5.3 instance boundary authority |
| Skill Definition Standard | enterprise/process/controls/ctrl-skill-definition-standard.md | Domain sprint prompt; L1–L6 hierarchy; mandatory inclusions |
| Constraint Library | enterprise/process/controls/ctrl-constraint-library.md | 120+ named constraint codes |
| Mechanism Catalog | enterprise/process/controls/ctrl-mechanism-catalog.md | 74+ named mechanism codes |
| Interface Register | enterprise/process/interfaces/ctrl-interface-register.md | 36+ cross-function and intercompany interface definitions |
| Instance Governance | enterprise/process/controls/ctrl-instance-governance-standard.md | Layered authority model, exchange protocol, onboarding sequence |
| SRS Standard | enterprise/process/controls/ctrl-srs-standard.md | SRS seed format, transfer protocol, instance ownership model |
...
Full interface register: `enterprise/process/interfaces/ctrl-interface-register.md`
